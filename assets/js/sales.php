...
<script type="text/javascript">
    ...
    function calculate_total() {
        ...
        $.each(rows, function () {
			...
			var val5 = parseFloat($(this).find('div.rate input').val());
			//alert(val5);
            _amount = parseFloat($(this).find('div.rate input').val()) * quantity;
           // alert(_amount);
            $(this).find('div.amount').html(_amount);
            subtotal += _amount;
			//alert(subtotal);
			return false;
            row = $(this);
	...
	// Append the added items to the preview to the table as items
    function add_item_to_table2(data, itemid, merge_invoice) {
        // If not custom data passed get from the preview
        if (typeof(data) == 'undefined' || data == 'undefined') {
            data = get_main_values();
        }
        ...
        var item_key = $('body').find('tbody .item').length + 1;
        table_row += '<tr class="sortable item" data-merge-invoice="' + merge_invoice + '">';
        table_row += '<td class="dragger"><div class="row"><div class="col-lg-2 col-sm-8"><label class="formobile">Item Name: </label>';
        // Check if quantity is number
        if (isNaN(data.qty)) {
            data.qty = 1;
        }
        ...
        get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
            ...
            table_row += '<input type="hidden" class="order" name="items[' + item_key + '][order]"><input type="hidden" name="items[' + item_key + '][saved_items_id]" value="' + data.new_itmes_id + '"><input type="hidden" name="new_itmes_id[]" value="' + data.new_itmes_id + '">';
           // table_row += '</div>';
            table_row += '<textarea  name="items[' + item_key + '][item_name]" class="form-control">' + data.item_name + '</textarea></div>';
            table_row += '<div class="col-lg-2 col-sm-8"><label class="formobile">Description: </label> <textarea  name="items[' + item_key + '][item_desc]" class="form-control item_item_desc" >' + data.item_desc.replace(regex, "\n") + '</textarea></div>';
            <?php $invoice_view = config_item('invoice_view');
            if (!empty($invoice_view) && $invoice_view == '2') {?>
            table_row += ' <div class="col-lg-2 col-sm-8"><input type="text" name="items[' + item_key + '][hsn_code]" class="form-control" value="' + data.hsn_code + '"></div>';
            <?php }?>
            table_row += '<div class="col-lg-1 col-sm-8"><label class="formobile">Qty/ Hours:</label><input type="text" data-parsley-type="number" min="0" onblur="calculate_total();" onchange="calculate_total();" data-quantity name="items[' + item_key + '][quantity]" value="' + data.qty + '" class="form-control">';
            ...
            table_row += '</div>';
            table_row += '<div class="col-lg-2 col-sm-8 rate"><label class="formobile">Price:</label><input type="text" data-parsley-type="number"  onblur="calculate_total();" onchange="calculate_total();" name="items[' + item_key + '][unit_cost]" value="' + data.rate + '" class="form-control"></div>';
            table_row += '<div class="col-lg-2 col-sm-8"><label class="formobile">Tax Rate:</label><div style="width: 100% !important;">' + tax_dropdown + '</div></div>';
            table_row += '<div class="col-lg-2 col-sm-8 amount"><label class="formobile">Total:</label>' + amount + '</div>';
            table_row += ' <div class="col-lg-1 col-sm-8"><label class="formobile">Action:</label><a href="#" class="btn-xs btn btn-danger pull-left" onclick="delete_item(this,' + itemid + '); return false;"><i class="fa fa-trash"></i></a></div></td>';
            table_row += '</tr>';
