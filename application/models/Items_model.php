...
class Items_Model extends MY_Model
{
                    ...
                    foreach ($tasks as $task_id) {
                        ...
                        if (!empty($total_tasks_timer)) {
                            foreach ($total_tasks_timer as $v_task_timer) {
                                ...
                                $description = lang('start_date') . ' : ' . strftime(config_item('date_format'), strtotime($v_task_timer->start_time)) . ' ' . display_time($v_timer->start_time, true) . '&nbsp';
                                $description .= lang('end_date') . ' : ' . strftime(config_item('date_format'), strtotime($v_task_timer->end_time)) . ' ' . display_time($v_timer->end_time, true) . '&nbsp';
    ...
    public function get_all_project($filterBy = null)
    {
        ...
        foreach ($all_projects as $v_projects) {
            ...
            if ($v_projects->project_status == $filterBy) {
                array_push($projects, $v_projects);
            }
        }
        return $projects;
