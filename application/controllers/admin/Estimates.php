...
class Estimates extends Admin_Controller
{
    ...
    public function index($action = NULL, $id = NULL, $item_id = NULL)
    {
        ...
        // get all estimate
        $data['all_estimates_info'] = $this->estimates_model->get_estimates($filterBy);
    ...
    function cloned_estimate($id)
    {
        ...
        if (!empty($can_edit) && !empty($edited)) {
            ...
            $new_invoice = array(
                ...
                'estimate_date' => $invoice_info->estimate_date,
                'estimate_month' => $invoice_info->estimate_month,
                'estimate_year' => $invoice_info->estimate_year,
...
//            redirect($_SERVER['HTTP_REFERER']);
    ...
    public function send_estimates_email($estimates_id, $row = null)
    {
        if (!empty($row)) {
